...
$servicos = filter_input(INPUT_POST, 'servicos', FILTER_UNSAFE_RAW);
...
if ($servicos != ' ') {
  if ($servicos == 'E') {
    $query_events .= " AND t.tps_codigo <> 2 ";
  } else {
    $query_events .= " AND t.tps_codigo = 2 ";
  }
} else {
  if($servicos == 'E'){
    $query_events = str_replace('AND t.tps_codigo <> 2' , '', $query_events);
  } else {
    $query_events = str_replace('AND t.tps_codigo = 2' , '', $query_events);
  }
...
if ($status != ''){
  if ($status == '1') {
    $query_events .= " AND a.sta_codigo =  1 AND a.tpg_codigo <> 6";
  } else {
    $query_events .= " AND a.sta_codigo = '" . $status . "' ";
  }
} else {
  $query_events .= " AND a.sta_codigo <> 3";
}
...
while ($row_events = $resultado_events->fetch(PDO::FETCH_ASSOC)) {
  ...
  if ($row_events['age_especificacao'] != "") {
    $extendedProps['especificacao'] = $row_events['age_especificacao'];
  } else {
    $extendedProps['especificacao'] = "Não especificado";
  }

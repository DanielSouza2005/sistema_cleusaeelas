...
$resultData = "select count(age_codigo) as agendamentos, date_format(age_data,'%d/%m/%Y') as data_atendimento
  ...
  if ($dataInicial != '' && $dataFinal != '')
    $resultData .= " AND a.age_data BETWEEN  '" . $dataInicial . "' AND '" . $dataFinal . "' ";

  if ($clientes != '')
    $resultData .= " AND c.cli_nome like '%" . $clientes . "%' ";
  if ($servicos != ' ') {
    if ($servicos != '2') {
      $resultData .= " AND t.tps_codigo <> 2 ";
    } else {
      $resultData .= " AND t.tps_codigo = '" . $servicos . "' ";
    }
  }
  if ($status != ''){
      $resultData .= " AND s.sta_codigo = '" . $status . "' ";
  }

  $resultData .= "group by age_data";
